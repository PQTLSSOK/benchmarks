...
$OPENSSL_BIN req -x509 -new -newkey "$ALG" -keyout "cacerts/$ALG/CA.key" -out "cacerts/$ALG/CA.crt" -nodes -subj "/CN=Benchmark CA" -days 365 -config "$OPENSSL_CONFIG"

$OPENSSL_BIN req -new -newkey "$ALG" -keyout "servercerts/$ALG/server.key" -out "servercerts/$ALG/server.csr" -nodes -subj "/CN=$SERVER_DOMAIN" -config "$OPENSSL_CONFIG"
